...
class TestPricing:
    ...
    def test_it_matches_the_figure_reconciled_against_the_bill(self):
        ...
        row = {"model": "google/gemini-3.1-flash-lite", "bucket": "x",
               "prompt_tokens": 50_096, "completion_tokens": 8_748}
        assert price_row(row, self.PRICES) == pytest.approx(0.0256, abs=0.0002)

    def test_a_free_slot_costs_nothing(self):
        row = {"model": "nvidia/nemotron-3-super-120b-a12b:free", "bucket": "x",
    ...
    def test_an_unknown_model_is_free_not_an_error(self):
        ...
        that cost nothing."""
        row = {"model": "some/slot-we-never-paid-for", "bucket": "x",
               "prompt_tokens": 9_000, "completion_tokens": 3_000}
        assert price_row(row, self.PRICES) == 0.0
    ...
    def test_purpose_rows_carry_no_model_and_price_at_zero(self):
        ...
        row = {"model": None, "bucket": "sitrep_country",
...
class TestRendering:
    ...
    def test_the_reconciliation_line_appears_when_the_key_answers(self, monkeypatch):
        ...
        out = m.render_money(
            [{"model": "a/b", "bucket": "a/b", "prompt_tokens": 1_000_000,
              "completion_tokens": 0}], 7, "model")
