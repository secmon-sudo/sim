...
def emit_sql(catalog: dict[str, int], cap: int, mode: str) -> None:
    """Print the catalog migration for this rescale, derived from rescale() itself.

    Hand-writing the UPDATE is how a replay and the thing it measured drift apart: the
    2026-08-16 anchor repair shipped with the resolver logic as it stood that morning
    and had to be superseded a day later. Generating it here means the numbers in the
    report and the values in the table come from one function.
    """
    changes = [(code, old, rescale(old, cap, mode))
               for code, old in sorted(catalog.items())]
    changes = [c for c in changes if c[1] != c[2]]

    print("-- ==========================================================")
    print(f"-- SIM - severity catalog rescale (mode={mode}, cap={cap})")
    print("--")
    print("-- GENERATED by scripts/replay_severity.py --emit-sql. Do not hand-edit:")
    print("-- regenerate it so the applied values and the replayed values stay one")
    print("-- function apart, not two transcriptions apart.")
    print("--")
    print(f"-- {len(changes)} of {len(catalog)} event types move. Every base at or")
    print(f"-- below {COMPRESS_KNEE} is left alone; above it the range is squeezed")
    print(f"-- into the room under {cap}, preserving the order between types.")
    print("--")
    print("-- Idempotent: re-running matches the same rows and writes the same values.")
    print("-- ==========================================================")
    print()
    print("BEGIN;")
    print()
    for code, old, new in changes:
        print(f"UPDATE event_type_catalog SET severity_base = {new:3d}  "
              f"-- was {old:3d}\n WHERE code = '{code}';")
    print()
    print("-- Should report 0 rows: every type at or above the knee has moved.")
    print("SELECT code, severity_base FROM event_type_catalog")
    print(f" WHERE severity_base > {cap} ORDER BY severity_base DESC;")
    print()
    print("COMMIT;")
...
def main() -> None:
    ...
    ap.add_argument("--emit-sql", action="store_true",
                    help="Print the UPDATE statements that apply this rescale to "
                         "event_type_catalog, instead of the delta report.")
    ...
    if args.emit_sql:
        emit_sql(catalog, args.base_cap, args.mode)
        return
